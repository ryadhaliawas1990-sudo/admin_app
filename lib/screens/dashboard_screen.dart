...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Future<void> loadStats() async {
    try {
      ...
      final people = await db.rawQuery("SELECT COUNT(DISTINCT number) as count FROM timeline");
      final records = await db.rawQuery("SELECT COUNT(*) as count FROM timeline");
      final years = await db.rawQuery("SELECT COUNT(DISTINCT year) as count FROM timeline");
      ...
      final statuses = await db.rawQuery('''
        ...
        WHERE year = ? AND month = ?
          AND status IS NOT NULL AND TRIM(status) != '' AND status != '-'
        GROUP BY status
        ORDER BY count DESC
  ...
  Future<void> exportStatusExcel(String status) async {
    ...
    final data = await db.query(
      'timeline',
      where: 'status = ? AND year = ? AND month = ?',
    ...
    sheet.appendRow([
      ex.TextCellValue("الرقم"), ex.TextCellValue("الاسم"), ex.TextCellValue("الرتبة"),
      ex.TextCellValue("الوحدة"), ex.TextCellValue("الحالة"), ex.TextCellValue("الشهر"), ex.TextCellValue("السنة"),
    ]);
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("تم تصدير ملف: $status")));
  }
            ...
            : ListView(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(labelText: "بحث", border: OutlineInputBorder()),
                    ...
                    SizedBox(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return ListTile(
                            title: Text("${item['number'] ?? ''} - ${item['rank'] ?? ''} - ${item['name'] ?? ''}"),
                            subtitle: Text("الحالة: ${item['status'] ?? ''} | الشهر: ${item['month'] ?? ''}"),
                          );
                        },
                      ),
                    ),
                  ...
                  Align(
                    ...
                    child: Text("الحالات (آخر شهر: $latestMonth / $latestYear)", style: const TextStyle(fontWeight: FontWeight.bold)),
                  ),
                  const SizedBox(height: 10),
                  SizedBox(
                    height: 300,
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return InkWell(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(12), border: Border.all()),
                            child: Column(
