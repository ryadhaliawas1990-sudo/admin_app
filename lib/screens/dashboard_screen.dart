  ...
  Future<void> loadStats() async {
    try {
      ...
      final people = await db.rawQuery(
        "SELECT COUNT(DISTINCT number) as count FROM timeline",
      );
      ...
      final years = await db.rawQuery(
        "SELECT COUNT(DISTINCT year) as count FROM timeline",
      );
      ...
      setState(() {
        totalPeople =
            int.tryParse(people.first['count'].toString()) ?? 0;

        totalRecords =
            int.tryParse(records.first['count'].toString()) ?? 0;

        totalYears =
            int.tryParse(years.first['count'].toString()) ?? 0;
